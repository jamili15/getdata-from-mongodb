...
export async function getHandler(req: NextApiRequest, res: NextApiResponse<ResponseData | ResponseError>, services: {}) {
  try {
...
export async function postHandler(req: NextApiRequest, res: NextApiResponse<ResponseData | ResponseError>, services: {}) {
  try {
    const { serviceName, method, data, option } = req.body;
    const result = await executeService(services, serviceName, method, data, option);
    return res.status(200).json({ status: "SUCCESS", data: result });
  } catch (error) {
    return res.status(401).json({ code: "01", error: error + "" });
  }
...
async function executeService(services: Record<string, any> = {}, serviceName: string, method: string, data = {}, option: Record<string, any> = {}) {
  console.log(`\n\n>>>${serviceName}.${method}<<<`);
  console.log(data);
  console.log("=================================\n\n");
